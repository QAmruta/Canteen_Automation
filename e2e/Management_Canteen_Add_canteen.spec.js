...
// add name of canteen uniquie for every test run
await page.getByRole('textbox', { name: 'Canteen Name*' }).click();
await page.getByRole('textbox', { name: 'Canteen Name*' }).fill('ABS canteen');
//add phone number of canteen uniquie for every test run
await page.getByRole('textbox', { name: 'Phone number must be 10' }).click();
await page.getByRole('textbox', { name: 'Phone number must be 10' }).fill('9496779678');
//add address of canteen
await page.getByRole('textbox', { name: 'Canteen Address*' }).click();
await page.getByRole('textbox', { name: 'Canteen Address*' }).fill('Nashik-10002');
